...
It seems that there is no good IRC plugin for vim - I found none of them at least. But there is an brilliant alternative for the all geeks - [ii][ii_site]. Here is a quote from site:
...
To configure the IRC workflow (join, identify, read/write) you can use [this][nion] [posts][userbound]. I will be saying only about some caveats in this process.

First of all I will show you the result:
<img src="/public/img/ii.png" border="0" width="100%">

I'll be using [tmux][tmux] + [multitail][multitail] + vim.

At the start we need connect to an IRC server (`freenode.net` in my case):
{% highlight bash %}
#!/bin/sh
...
Now we should compose a convenient environment for the reading/writing. A small bash script can be used for this purpose (I've split the implementation):
{% highlight bash %}
#!/bin/sh
# tmux_open.sh
...
We should use `-2` option for `tmux` to force 256 colors, and `-cS ii` option for `multitail` to ii syntax highlighting. After all this we can execute `./tmux_open.sh channel` command to open a two pane, that will be contain a channel log and vim.

To type in IRC session we will be using `vim` with the following mappings:
{% highlight vim %}
...
Also we can hide tmux status line globally (I prefer a vim status line) to achive an ideal:
{% highlight bash %}
# .tmux.conf
